      ...
      <div className="relative" ref={projectRef}>
        ...
        {projectOpen && (
          <div className="absolute left-0 top-full z-50 mt-2 w-64 rounded-lg border border-line bg-card p-1.5">
            <div className="px-2 py-1.5 font-mono text-[10px] uppercase tracking-wider text-ink-40">
              切换项目
            </div>
            {projects.length === 0 ? (
              <div className="px-2 py-3 font-mono text-xs text-ink-40">
                暂无项目
              </div>
            ) : (
              projects.map((p) => (
          ...
          {bellOpen && (
            <div className="absolute right-0 top-full z-50 mt-2 w-80 rounded-lg border border-line bg-card overflow-hidden">
              ...
              <div className="flex items-center justify-between border-b border-line-soft px-4 py-2.5">
                ...
                {unread > 0 && (
                  <span className="font-mono text-[10px] text-ink-40">{unread} 条未读</span>
                )}
              </div>
              ...
              {alerts.length === 0 ? (
                <div className="px-4 py-8 text-center">
                  <div className="font-mono text-xs text-ink-40">暂无预警</div>
                </div>
              ) : (
                <div>
                  {alerts.map((a, idx) => (
              ...
              <div className="flex items-center justify-between border-t border-line-soft px-4 py-2.5">
                {unread > 0 ? (
                  <button
                    ...
                    className="font-mono text-[11px] text-ink-60 hover:text-ink"
                  >
                    全部标为已读
                  </button>
                ) : (
                  <span className="font-mono text-[11px] text-ink-40">全部已读</span>
                )}
                <Link
                  href="/app"
                  onClick={() => setBellOpen(false)}
                  className="font-mono text-[11px] text-brand hover:text-brand-deep"
                >
                  查看全部 →
                </Link>
              </div>
          ...
          {userMenuOpen && (
            <div className="absolute right-0 top-full z-50 mt-2 w-56 rounded-lg border border-line bg-card overflow-hidden">
              ...
              {isAuthEnabled ? (
                ...
                <div className="px-4 py-2.5 font-mono text-[10px] text-ink-40">
                  演示模式 · 账号系统未启用
                </div>
              )}
            </div>
