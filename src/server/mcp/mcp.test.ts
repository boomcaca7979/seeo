...
describe("SeeO MCP standard Streamable HTTP compatibility", () => {
  it("connects with the official MCP client and lists exactly nine tools", async () => {
    const { client } = await officialClient();
    const listed = await client.listTools();
    expect(listed.tools.map((tool) => tool.name)).toEqual([
      "list_projects", "project_context", "research_keywords", "get_serp_results", "get_backlinks_profile", "get_rank_history", "ai_search_brand_lookup", "get_competitor_keyword_gap", "search_console_tools",
    ]);
    await client.close();
  });
