...
export async function generateWeeklyReport(): Promise<WeeklyReportResult> {
  ...
  // 1. 排名变化摘要
  const rankChanges = await getRankChangesSince(sinceISO);
  ...
  // 2. 审计摘要（取最近一周）
  const recentAudits = await getAuditsSince(sinceISO);
  ...
  // 3. 关键词追踪概况
  const totalKeywords = await countTrackedKeywords();
  const activeKeywords = await countActiveKeywords();
