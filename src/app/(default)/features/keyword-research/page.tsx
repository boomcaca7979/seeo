...
// ===== /features/keyword-research 内容组件（en / zh）=====
// 文案走 messages（keywordResearch / featureShared），metadata 由 [locale] 页面按 locale 生成。

export default async function KeywordResearchFeaturePage() {
  const t = await getTranslations("keywordResearch");
  const s = await getTranslations("featureShared");
  const locale = (await getLocale()) as "en" | "zh";
  const path = "/features/keyword-research";
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <JsonLd schema={featureAppSchema("keywordResearch", lpath, locale)} />
      <Navbar />
      ...
      <div className="doc-shell px-6 py-16">
        ...
        <div className="mt-12 grid gap-3 sm:grid-cols-2">
          <Link
            href={localePath(locale, "/features/competitor-analysis")}
          ...
          >
            ...
            <h3 className="mt-1 font-display text-sm font-semibold text-ink">
              {t("related.competitorAnalysis.title")}
            </h3>
            <p className="mt-1 font-sans text-xs text-ink-60">
              {t("related.competitorAnalysis.desc")}
            </p>
          </Link>
          <Link
            href={localePath(locale, "/features/content-optimization")}
            className="card-a p-4 transition-colors hover:border-brand"
          >
            <span className="font-mono text-xs text-brand">{s("related")}</span>
            <h3 className="mt-1 font-display text-sm font-semibold text-ink">
              {t("related.contentOptimization.title")}
            </h3>
            <p className="mt-1 font-sans text-xs text-ink-60">
              {t("related.contentOptimization.desc")}
            </p>
          </Link>
        </div>
