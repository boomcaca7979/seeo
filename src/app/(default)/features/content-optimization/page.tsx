...
// ===== /features/content-optimization 内容组件（en / zh）=====
// 文案走 messages（contentOptimization / featureShared），metadata 由 [locale] 页面按 locale 生成。

export default async function ContentOptimizationFeaturePage() {
  const t = await getTranslations("contentOptimization");
  const s = await getTranslations("featureShared");
  const locale = (await getLocale()) as "en" | "zh";
  const path = "/features/content-optimization";
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <JsonLd schema={featureAppSchema("contentOptimization", lpath, locale)} />
      <Navbar />
